...
CREATE TABLE tb_tipo_redes_sociais (
    id_tipo_redes_sociais INT AUTO_INCREMENT PRIMARY KEY,
    nome VARCHAR(100) NOT NULL
);
...
CREATE TABLE tb_usuario (
    ...
    FOREIGN KEY (nacionalidade_id) REFERENCES tb_nacionalidade(id_nacionalidade),
    FOREIGN KEY (endereco_id) REFERENCES tb_endereco(id_endereco),
    FOREIGN KEY (sexo_id) REFERENCES tb_genero(id_genero)
);
...
CREATE TABLE tb_usuario_foto (
    ...
    FOREIGN KEY (foto_id) REFERENCES tb_foto(id_foto) ON DELETE CASCADE,
    FOREIGN KEY (usuario_id) REFERENCES tb_usuario(id_usuario) ON DELETE CASCADE
);
...
CREATE TABLE tb_redes_sociais (
    id_redes_sociais INT AUTO_INCREMENT PRIMARY KEY,
    link VARCHAR(255) NOT NULL,
    tipo_id INT NOT NULL,
    usuario_id INT NOT NULL,

    FOREIGN KEY (tipo_id) REFERENCES tb_tipo_redes_sociais(id_tipo_redes_sociais),
    FOREIGN KEY (usuario_id) REFERENCES tb_usuario(id_usuario) ON DELETE CASCADE
);
...
CREATE TABLE tb_evento_organizador (
    ...
    FOREIGN KEY (evento_id) REFERENCES tb_evento(id_evento) ON DELETE CASCADE,
    FOREIGN KEY (organizador_id) REFERENCES tb_organizador(id_organizador) ON DELETE CASCADE
);
...
CREATE TABLE tb_artista_genero (
    ...
    FOREIGN KEY (genero_id) REFERENCES tb_genero(id_genero),
    FOREIGN KEY (artista_id) REFERENCES tb_artista(id_artista) ON DELETE CASCADE
);
...
CREATE TABLE tb_evento_artista_status (
    id_evento_artista_status INT AUTO_INCREMENT PRIMARY KEY,
    evento_artista_id INT NOT NULL,
    status_id INT NOT NULL,
    data_hora DATETIME NOT NULL,

    FOREIGN KEY (evento_artista_id) REFERENCES tb_evento_artista(id_evento_artista) ON DELETE CASCADE,
    FOREIGN KEY (status_id) REFERENCES tb_status(id_status)
);
...
CREATE TABLE tb_avaliacao_evento (
    id_avaliacao_evento INT AUTO_INCREMENT PRIMARY KEY,
...
CREATE TABLE tb_avaliacao_artista (
    id_avaliacao_artista INT AUTO_INCREMENT PRIMARY KEY,
    numero_estrelas INT NOT NULL,
    usuario_id INT NOT NULL,
    artista_id INT NOT NULL,
    data_avaliacao DATETIME DEFAULT CURRENT_TIMESTAMP,
